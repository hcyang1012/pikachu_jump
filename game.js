...
let gameState = {
    ...
    jumpCooldown: detectMobile() ? 5 : 600, // 모바일: 5ms (더블/트리플 점프용), PC: 600ms
...
function handleTouchStart(e) {
    ...
    const cooldown = detectMobile() ? 30 : 150; // 모바일 쿨다운 30ms
...
// 모바일에서 더블 탭 줌 방지 및 터치 민감도 조절 (body에도 적용)
...
function jump() {
    ...
    // 다섯 번째 점프 (공중에서, 쿨다운 확인)
    else if (gameState.jumpCount === 4 && 
             currentTime - gameState.lastJumpTime >= gameState.jumpCooldown) {
        player.velocityY = player.jumpPower;
        gameState.jumpCount++;
        gameState.lastJumpTime = currentTime;
    ...
    // 여섯 번째 점프 (공중에서, 쿨다운 확인)
    else if (gameState.jumpCount === 5 && 
             currentTime - gameState.lastJumpTime >= gameState.jumpCooldown) {
        player.velocityY = player.jumpPower;
        gameState.jumpCount++;
        gameState.lastJumpTime = currentTime;
...
function restartGame() {
    gameState = {
        ...
        maxJumps: 6, // 6단 점프 지원
        lastJumpTime: 0,
        jumpCooldown: detectMobile() ? 5 : 600,
